...
class Billing extends Component
{
    ...
    {
        if ($this->lastSaleId) {
            $url = route('receipts.print', ['id' => $this->lastSaleId]);
            $this->js("window.open('{$url}', '_blank', 'width=1000,height=600')");
        } else {
            $this->dispatch('showToast', ['type' => 'error', 'message' => 'No receipt to print.']);
        }
    }
     ...
     */
    public function getFilePreviewInfo($file, $previewUrl = null): ?array
    {
        if (!$file) {
            return null;
        }
