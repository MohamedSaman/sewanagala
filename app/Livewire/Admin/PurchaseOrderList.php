...
class PurchaseOrderList extends Component
{
    ...
    public function render()
    {
        ...
        $query = $query->orderByRaw("FIELD(status, 'pending', 'received', 'complete', 'cancelled')")
            ->orderBy('id', 'desc');

        if ($this->perPage === 'all') {
            $totalRows = (clone $query)->count();
            $orders = $query->paginate($totalRows > 0 ? $totalRows : 1);
        } else {
    ...
    public function exportCSV()
    {
        ...
        $headers = [
            ...
            "Content-Disposition" => "attachment; filename=purchase_orders_" . now()->format('YmdHis') . ".csv",
            "Pragma"              => "no-cache",
        ...
        return response()->streamDownload($callback, 'purchase_orders_' . now()->format('YmdHis') . '.csv', $headers);
    }
}
