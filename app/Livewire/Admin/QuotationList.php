...
class QuotationList extends Component
{
    ...
    public function render()
    {
        ...
        if ($this->perPage === 'all') {
            ...
            $quotations = $query->paginate($totalRows > 0 ? $totalRows : 1);
        } else {
            $quotations = $query->paginate((int) $this->perPage);
        }
    ...
    public function exportCSV()
    {
        $query = Quotation::with('customer')
            ->when($this->search, function ($q) {
                $q->where(function ($sub) {
                    $sub->where('quotation_number', 'like', '%' . $this->search . '%')
                        ->orWhere('customer_name', 'like', '%' . $this->search . '%')
                        ->orWhere('customer_phone', 'like', '%' . $this->search . '%');
                });
            })
        ...
        $headers = [
            ...
            "Content-Disposition" => "attachment; filename=quotations_" . now()->format('YmdHis') . ".csv",
            "Pragma"              => "no-cache",
            "Cache-Control"       => "must-revalidate, post-check=0, pre-check=0",
            "Expires"             => "0"
        ];

        $columns = ['Quotation Number', 'Date', 'Customer', 'Phone', 'Sub Total', 'Discount Amount', 'Total Amount'];
        ...
        $callback = function() use($quotations, $columns) {
            ...
            foreach ($quotations as $quotation) {
                fputcsv($file, [
                    ...
                    $quotation->customer->name ?? $quotation->customer_name ?? 'Walk-in Customer',
        ...
        return response()->streamDownload($callback, 'quotations_' . now()->format('YmdHis') . '.csv', $headers);
    }
}
