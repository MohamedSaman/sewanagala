...
class AdminDashboard extends Component
{
    ...
    public function loadStockStats(): void
    {
        ...
        // Calculate percentages based on the new total stock
    ...
    {
        // Get sold quantities per product from sale_items
        $soldCounts = DB::table('sale_items')
            ->select('product_id', DB::raw('SUM(quantity) as sold_qty'))
            ->groupBy('product_id');

        // Join Products and stock tables to get full inventory data
        $this->ProductInventory = DB::table('product_details')
            ->join('product_stocks', 'product_details.id', '=', 'product_stocks.product_id')
            ->join('brand_lists', 'product_details.brand_id', '=', 'brand_lists.id')
            ->leftJoinSub($soldCounts, 'sold_items', function ($join) {
                $join->on('product_details.id', '=', 'sold_items.product_id');
            })
            ->select(
                'product_details.id',
                ...
                DB::raw('(product_stocks.available_stock + product_stocks.damage_stock + COALESCE(sold_items.sold_qty, 0)) as total_stock')
            )
