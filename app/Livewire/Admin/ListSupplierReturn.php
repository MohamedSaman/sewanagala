...
class ListSupplierReturn extends Component
{
    ...
    {
        $returnsQuery = $this->getReturnsQuery();

        if ($this->perPage === 'all') {
            $totalRows = (clone $returnsQuery)->count();
            $returns = $returnsQuery->paginate($totalRows > 0 ? $totalRows : 1);
        } else {
            $returns = $returnsQuery->paginate((int) $this->perPage);
        }
